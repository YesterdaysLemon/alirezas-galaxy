...
The galaxy's camera position, orientation and visibility are saved on entry and restored on return, including after switching systems or cancelling an entry. Patterns & Life holds seven worlds, D. melanogaster beside C. elegans and Dodeca & Point outermost. Curiosity & Play fills its eight lanes, with the ringed gas giant SOFT SIGNAL outermost behind a frost-line asteroid belt. Tools & Infrastructure has seven worlds. Ideas & Inquiry, around the star Vesper, holds Lyrebird, The Intuition Lab, please. and The Talking Cure. Families Jev opened hold the rest: Worlds & Games (Morphogenesis, Openwater, Nightfall Bunker), Places & Maps (Between Worlds) and Machines & Minds (Next-Token Machine). Frontier, where worlds Jev is unsure about wait, holds Coolimages Museum; its star appears only when populated. Every world lives under a visible star: a full family opens a new one rather than spilling into a hidden sister system (see [daily discovery](world-discovery.md)). Sparse systems frame at a common minimum scale.
...
Each project has a permanent family `systemId` and family-wide `orbitSlot`. Eight lanes form a system: slots 0–7 use `<family>`, 8–15 use `<family>-2`, and so on. Removing a member leaves a gap rather than moving its neighbors. Entire empty companions disappear from the star-system list without renumbering later systems. Sister systems exist only as an overflow the daily refresh drains, and a family star's label counts the worlds its own system shows. New discoveries receive Frontier slots; catalog size no longer depends on a galaxy marker cap. The discovery state retains retired addresses so recovery and future growth do not reshuffle worlds. See [daily discovery](world-discovery.md) for public eligibility and curation.
