...
Public eligibility is independent of galaxy marker capacity. The discovery script retains HTTPS/public opt-in, denials and health grace, assigns new discoveries permanent Frontier addresses, and retains retired address tombstones. Every world lives under a visible star: a family whose eight lanes are used is full, and Jev opens a themed family instead; the daily refresh re-homes discovered worlds from hidden sister systems and Frontier when the families change, recording each old address in `formerAddresses`. Reviewed registry worlds stay within their family's first eight lanes (unit-tested). Never compact slots or reuse retired or former addresses. The overview framing fits the outermost orbit clear of the measured HUD (`fitOverview` in `lib/solar/framing.ts`). See `docs/SOLAR-SYSTEMS.md` and `docs/world-discovery.md`. Pushes to main trigger production: publishing, merging and deployment require explicit authorization. Use `vps-operations` for authorized Deploy Manager work and `credentials-access` when service authentication is needed.
